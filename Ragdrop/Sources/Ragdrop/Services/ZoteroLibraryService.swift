...
struct ZoteroLibraryService: Sendable {
    ...
    func fetchPDFs() async throws -> [ZoteroPDF] {
        ...
        let documents: [ZoteroPDF] = attachments.compactMap { attachment -> ZoteroPDF? in
            guard let enclosure = attachment.links?.enclosure,
        ...
        return await Task.detached(priority: .utility) {
            documents.map { document in
                var copy = document
                copy.fingerprintPrefix = try? Self.sha256Prefix(of: document.fileURL)
                return copy
            }
        }.value
    }
    ...
    private static func sha256Prefix(of url: URL) throws -> String {
